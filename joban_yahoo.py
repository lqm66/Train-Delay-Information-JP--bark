...
def fetch_page_info(name, url):
    """
    解析每一条线路页面，返回:
        updated: '11月28日 9時52分更新'
        status : '平常運転', '列車遅延' 等
        reason : 原因（平常運転时为 None）
    """
    ...
    if title_idx is not None and title_idx + 1 < len(strings):
        updated = strings[title_idx + 1]  # e.g. 11月28日 8時59分
        if title_idx + 2 < len(strings) and "更新" in strings[title_idx + 2]:
            updated += strings[title_idx + 2]  # + 更新
        elif "更新" not in updated:
            updated += "更新"

    # 如果失败，就在全文里找一次“xx月xx日 xx時xx分”
    if updated == "更新時刻不明":
    ...
    if status_idx is not None and "平常運転" not in status:
        ...
        if detail_lines:
            # 典型：26 行红字原因 + 27 行（11月xx日 xx時xx分掲載）
...
def build_grouped_message(results):
    """
    按 (status, reason) 分组，一样的合并标题，不一样的拆开。
    返回:
        has_abnormal  是否有非平常運転（且非情報取得エラー）
        has_severe    是否有 運転見合わせ / 運休 / 脱線
        body          Bark 正文（不含最上面的「常磐線運行情報」）
    """
    # 1. 分组
    groups = {}
    for r in results:
...
def send_bark(title, body, icon_url=None):
    ...
    resp.raise_for_status()  # 如果 Bark 返回错误，这一步会直接抛异常
